...
system.run(async () => {
    ...
    if (!await modules.get('combatTime')) modules.set('combatTime', 5);
    if (await modules.get('combatLog') === undefined) modules.set('combatLog', true);
    if (await modules.get('keepInventory') === undefined) modules.set('keepInventory', false);
    if (await modules.get('combatKillLeave') === undefined) modules.set('combatKillLeave', true);
    if (!await modules.get('combatMessage')) modules.set('combatMessage', 'You are in combat! Ending in {t}')
