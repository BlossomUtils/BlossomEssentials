...
mc.system.run(() => {
    ...
    mc.world.beforeEvents.chatSend.subscribe((msg) => {
        ...
        if (moduleAPI.chatranks === false) return;
        msg.cancel = true;
        mc.system.run(() => {
            ...
            msg.sender.runCommand(`scriptevent blossom:messageSent - [${joined.replaceAll(/§[0-9a-fgmnolr]/gi, "").replaceAll('\uE5A6', 'CLAN')}] ${msg.sender.name} >> ${msg.message}`)
        })
    })
})