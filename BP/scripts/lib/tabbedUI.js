...
export class TabUI {
    ...
    open(player, tab = 0, currentPage = Math.floor(tab / this.tabsPerPage)) {
        ...
        form.title(`§t§a§b§b§e§d§r§f${blossomFormatting.format(this.tabs[tab].title, player)}`);
        ...
        if (this.tabs.length <= 5) {
            for(let i = 0; i < this.tabs.length; i++) {
                form.button(`§t§a§b${i == tab ? "§a§c§t§i§v§e" : ""}§r§f${blossomFormatting.format(this.tabs[i].title, player)}`, null, (player) => {
...
uiManager.addUI("tab_test | Blossom/Tests/Tab", "a", (player)=>{
    tabUI.open(player)
})