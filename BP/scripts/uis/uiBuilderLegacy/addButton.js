...
uiManager.addUI(config.uiNames.UIBuilder.AddButton, "Add a button", (player, id, index = -1, data2, initial = true)=>{
    ...
    form.button(`§dSet display ${!data.text ? " §c*" : ""}\n§7Set the display of the button`, null, (player)=>{
        ...
        modalForm.show(player, false, (player, response)=>{
            if(response.canceled) return uiManager.open(player, config.uiNames.UIBuilder.AddButton, id, index, data, false);
            data.text = response.formValues[0];
            data.subtext = response.formValues[1];
            data.requiredTag = response.formValues[2];
            uiManager.open(player, config.uiNames.UIBuilder.AddButton, id, index, data, false);
        })
    })
